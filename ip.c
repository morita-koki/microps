...
struct ip_hdr {
    uint8_t vhl; // version(4bit)  +  header length(4bit) = 8bit
    uint8_t tos; // type of service
    uint16_t total; // total length
    uint16_t id; // identification
    uint16_t offset; // flag(3bit)  +  fragment offset(13bit) = 16bit
    uint8_t ttl; // time to live(8bit)
    uint8_t protocol; 
    uint16_t sum; // header checksum(16bit)
    ip_addr_t src; // source address(32bit) 送信元アドレス
    ip_addr_t dst; // destination address(32bit) 宛先アドレス
    uint8_t options[];
};
...
ip_dump(const uint8_t *data, size_t len)
{
    ...
    flockfile(stderr); // err出力の際に別プロセスのログが出力されないようにファイルの所有権を獲得
...
ip_iface_alloc(const char *unicast, const char *netmask)
{
    ...
    if (ip_addr_pton(unicast, &iface->unicast) == -1) {
        errorf("iP_addr_pton() failure, adrr=%s", unicast);
        memory_free(iface);
        return NULL;
    }
    if (ip_addr_pton(netmask, &iface->netmask) == -1) {
        errorf("ip_addr_pton() failute, addr=%s", netmask);
        memory_free(iface);
        return NULL;
    }
...
ip_protocol_register(uint8_t type, void (*handler)(const uint8_t *data, size_t len, ip_addr_t src, ip_addr_t dst, struct ip_iface *iface))
{
    ...
    for (entry = protocols; entry; entry = entry->next) {
        if (entry->type == type) {
            errorf("already exist, type=%u", type);
            return -1;
        }
    }
...
ip_input(const uint8_t *data, size_t len, struct net_device *dev)
{
    ...
    */
    ...
    total = ntoh16(hdr->total);
    if (len < total) {
        errorf("header length error: len=%zu < hlen=%u", len, total);
        return;
    }
...
ip_output(uint8_t protocol, const uint8_t *data, size_t len, ip_addr_t src, ip_addr_t dst)
{
    ...
    id = ip_generate_id();
    if (ip_output_core(iface, protocol, data, len, iface->unicast, nexthop, dst, id, 0) == -1) {
